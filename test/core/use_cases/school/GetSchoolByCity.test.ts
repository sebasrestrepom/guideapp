...
describe('GetSchoolByCity should', () => {
  ...
  test('save a new school', async () => {
    ...
    then_list_the_schools_with_this_information(school.cityId);
  });
  ...
  async function when_get_the_schools_with_this_data(cityId: number) {
    schools = await getSchoolByCity.execute(cityId);
  }

  async function then_list_the_schools_with_this_information(cityId: number) {
    await getSchoolByCity.execute(cityId);
    expect(schools).not.toBeNull();
    expect(schools).not.toBeUndefined();
  }
});
